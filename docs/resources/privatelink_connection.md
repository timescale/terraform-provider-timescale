...
subcategory: ""
description: |-
  Manages an Azure Private Link connection in a Timescale project.
  ...
  Workflow
  Create an Azure Private Endpoint pointing to the Timescale Private Link ServiceUse this resource with azure_connection_name set to the private endpoint nameThe resource will sync and wait for the connection to appearSet ip_address to the private IP from the Azure Private EndpointThe connection can then be used with timescale_service.private_endpoint_connection_id
  Important
...
- `ip_address` (String) The private IP address of the Azure Private Endpoint. This is required to enable services to connect via this private link.
- `region` (String) The Timescale region (e.g., az-eastus2).
